...
public class SkillZController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.GetKey(KeyCode.Z) && !isOnCooldown && sliderHp.GetCurrentMana() >= 30 && sliderHp.GetCurrentLevel() >= 20)
        {        
            isCharging = true;
            ChargeEnergy();
            FlipPlayerToCamera();
        ...
        if (Input.GetKeyUp(KeyCode.Z) && isCharging && sliderHp.GetCurrentMana() >= 30 && sliderHp.GetCurrentLevel() >= 20)
        {
